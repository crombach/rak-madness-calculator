...
- `functions/api/picks/index.ts` lists the seasons that have picks, and `functions/api/picks/[season]/[week].ts` reads `picks/<season>/<week>.xlsx` from the binding. Locally the bucket is simulated and empty, so `/api/picks` comes back with an empty list and `/api/picks/<season>/<week>` returns 404 until seeded. `wrangler.toml` carries the seed command.
