...
- `DialogShell.scss`: a bottom sheet, a centered modal at `wide-screen`, sized
  against `useViewportInsets` so a keyboard covers nothing. `--rak-dialog-inset`
  stands the bar on the rule. `--rak-dialog-floor` puts the keyboard's room
